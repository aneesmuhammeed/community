...
export default function AdminSidebarVisitor() {
  return (
    <div className="flex flex-col bg-background border-r border-border" style={{ width: '240px', minHeight: '100%' }}>
      ...
      <nav className="flex-1 px-3 py-4 flex flex-col gap-0.5">
        {navItems.map((item, i) => {
          const isActive = i === 2; // Visitor Control
          return (
            <a key={i} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium font-body cursor-pointer ${isActive ? 'bg-secondary text-primary' : 'text-muted-foreground'}`}>
